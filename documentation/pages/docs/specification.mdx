...
- publicData
  ...
  - snapshotId
  - parentSnapshotProof
  - parentSnapshotUpdatesClocks: `{ [clientPublicKey]: clock }`
- signature
...
When creating a snapshot the publicData must contain a `parentSnapshotUpdatesClocks`. In there the client should store the clocks it has been aware of, for others to verify later.
