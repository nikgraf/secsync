...
All messages (snapshots, updates & ephemeral messages) are encrypted using an AEAD construction and the resulting ciphertext, nonce and public data is then hashed. The result of this hash is signed.
...
In order to commit to one (plaintext, AAD) pair matching only one (ciphertext, authentication tag) pair every message is prefixed with a commitment tag.
It is generated from an `HMAC(nonce + ciphertext + AAD, key)`. Before every decryption the commitment tag is verified and an error thrown in case it's not valid.
...
- publicData
  ...
  - snapshotId
  - parentSnapshotProof
  - parentSnapshotUpdateClocks: `{ [clientPublicKey]: clock }`
- signature
...
When creating a snapshot the publicData must contain a `parentSnapshotUpdateClocks`. In there the client should store the clocks it has been aware of, for others to verify later.
...
An ephemeral message must include the referecening document ID, snapshot ID and if available the referencing update ID from the source client. In addition a counter stored in the encrypted content is included to prevent replay attacks for the same session. Preventing replay attacks for new sessions is out of scope. It can be by storing the ephemeral message counter for each snapshot and update in a local database.

An update consists of the

- ciphertext (containing the ephemeral message counter)
- nonce
- publicData
  - documentId
  - pubKey
